...
class ComicAdd(CreateView):
    '''add new comics to user's collection'''
    model = Comic
    ...
    def form_valid(self, form):
        ...
        for num in range(int(first), int(last)):
            cached_comic = Comic.objects.filter(series=data['series'], issue_number=num).first()
            print ("CACHED COMIC", cached_comic)
            if cached_comic:
                user_comic = UserComic.objects.create(comic_issue=cached_comic, user_id=self.request.user.id)
                print ("USER", user_comic)
                user_comic.save()
            else:
